...
public class WebUIManager extends BaseUIManager implements AdapterView.OnItemClickListener, View.OnClickListener, WebUIListener, MediaPlayerCallback {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 播放器
//        View listHeader = LayoutInflater.from(mContext).inflate(R.layout.list_header_layout, null);
        ucMediaPlayer = (UCMediaPlayer) ((Activity) mContext).findViewById(R.id.media_player);
        ucMediaPlayer.onCreate();
...
//        vListView.addHeaderView(listHeader);
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        VideoEntity videoModel = adapter.getItem(pos);
        if (videoModel == null) return;

        setTitle(videoModel);

        reset(videoModel.getPath(), videoModel.getCurrPos());
    }
    ...
    private void go2fullScreen() {
        if (!UCToastUtil.checkNetworkAndShowToastIfNecessary(mContext)) {
            return;
        }
