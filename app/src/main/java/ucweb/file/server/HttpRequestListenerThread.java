...
public class HttpRequestListenerThread extends Thread {
    ...
    private static final String TAG = HttpRequestListenerThread.class.getSimpleName();
    ...
    public HttpRequestListenerThread(int port, String path) throws IOException {
        serverSocket = new ServerSocket(port);

        HttpProcessor httpProc = new ImmutableHttpProcessor(new HttpResponseInterceptor[]{new ResponseDate(), new ResponseServer(),
                new ResponseContent(), new ResponseConnControl()});
        params = new BasicHttpParams();
        params.setIntParameter(CoreConnectionPNames.SO_TIMEOUT, 8000)
        ...
        registry.register("*", new WebServiceHandler(path));
