...
public class AbsVerticalSeekBar extends VerticalProgressBar {
     ...
     */
    protected boolean mIsUserSeekable = true;
    ...
    public AbsVerticalSeekBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        int thumbOffset = a.getDimensionPixelOffset(R.styleable.SeekBar_android_thumbOffset,
                getThumbOffset());
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // parameter does
        int thumbHoffset = thumb.getIntrinsicHeight()/2;
        ...
        if (thumbWidth > trackWidth) {
            ...
            if (d != null) {
                ...
                // start drawing
                d.setBounds(gapForCenteringTrack, 0+thumbHoffset, w - mPaddingRight - mPaddingLeft
                        - gapForCenteringTrack, h - mPaddingBottom - mPaddingTop-thumbHoffset);
            }
        } else {
            if (d != null) {
                // Canvas will be translated by the padding, so 0,0 is where we
                // start drawing
                d.setBounds(0, 0+thumbHoffset, w - mPaddingRight - mPaddingLeft, h - mPaddingBottom
                        - mPaddingTop-thumbHoffset);
            }
